...
import { CalendarRange, Lock, Unlock, Loader2 } from "lucide-react";
...
function CyclesPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        eyebrow="Cycle management"
        title="Goal cycles"
        description="Open/close cycles, configure quarterly windows, and manage exceptions."
      />
          ...
          <GlassCard key={c.name} className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-primary">Cycle</p>
                </div>
                <h3 className="mt-1 text-xl font-semibold">{c.name}</h3>
                <p className="text-xs text-muted-foreground">
                  {c.start} – {c.end} · {c.completion}% complete
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Switch
                  ...
                  onCheckedChange={() => toast.success(`${c.name} ${c.open ? "closed" : "opened"}`)}
                />
              </div>
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <Button
                ...
                onClick={() => toast.success(c.locked ? "Unlocked" : "Locked")}
              >
                {c.locked ? (
